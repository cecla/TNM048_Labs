...
function area(data) {
    ...
    var area = d3.svg.area()
            .interpolate("step")
            .x(function (d) {
                return x(format(d.time));//Complete the code
            })
            .y0(height)
            .y1(function (d) {
    ...
    //Creates the small chart        
        var area2 = d3.svg.area()
            .interpolate("step")
            .x(function (d) {
                return x2(format(d.time));//Complete the code
            })
            .y0(height2)
            .y1(function (d) {
    ...
    x.domain(d3.extent(data, function(d){ return format(d.time); }));//Complete the code
    y.domain([0, d3.max(data, function(d){ return d.mag; })]);//Complete the code
    ...
    function brush() {
        ...
        //Complete the code
        //Create an array with all the data points in the specified interval
        var filterBrush = data.filter(function(p){
            if(x.domain()[0] < format(p.time) && x.domain()[1] > format(p.time)){
                return p;
            }
        });
        map1.filterTime(filterBrush); //filter the map
    }
}
